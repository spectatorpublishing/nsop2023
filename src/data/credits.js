// Mock data to define all the people who work on a special edition
export const credits = 
    {
      "Corporate Board": [
        { staff_name: "Irie Sentner", title: "Editor-in-Chief" },
        { staff_name: "Andrew Park", title: "Managing Editor" },
        { staff_name: "Tyler Shern", title: "Publisher" }
      ],
      "Engineering" : [
        { staff_name: "Marian Abuhazi", title: "Engineering Manager" },
        { staff_name: "Darien Moment", title: "Engineering Manager" }
      ],
      "Product" : [
        { staff_name: "Muchen Guo", title: "Head of Product" }
      ],
      "University News": [
        {
          "staff_name": "Isabella Ramírez",
          "title": "University News Editor"
        },
        {
          "staff_name": "Esha Karam",
          "title": "University News Editor"
        },
        {
          "staff_name": "Sarah Huddleston",
          "title": "University News Deputy Editor"
        },
        {
          "staff_name": "Emily Forgash",
          "title": "University News Deputy Editor"
        },
        {
          "staff_name": "Shea Vance",
          "title": "University News Deputy Editor"
        },
        {
          "staff_name": "Amanda Chapa",
          "title": "University News Deputy Editor"
        },
        {
          "staff_name": "Sabrina Ticer-Wurr",
          "title": "University News Deputy Editor"
        }
      ],
      "Photo": [
        {
          "staff_name": "Judy Goldstein",
          "title": "Photo Editor"
        },
        {
          "staff_name": "Pip Tsuki Carlson",
          "title": "Photo Deputy Editor"
        },
        {
          "staff_name": "Gabriella Gregor Splaver",
          "title": "Photo Deputy Editor"
        },
        {
          "staff_name": "Salem Maru",
          "title": "Photo Deputy Editor"
        },
        {
          "staff_name": "Grace Li",
          "title": "Photo Deputy Editor"
        },
        {
          "staff_name": "Hannah Yanover",
          "title": "Photo Deputy Editor"
        },
        {
          "staff_name": "Esmeralda Paredes",
          "title": "Staff Photographer"
        },
        {
          "staff_name": "Isabel Iino",
          "title": "Staff Photographer"
        }
      ],
      "Sports": [
        {
          "staff_name": "Jorge Hernandez",
          "title": "Sports Editor"
        },
        {
          "staff_name": "Brian Lee",
          "title": "Deputy Sports Editor"
        },
        {
          "staff_name": "Takashi Williams",
          "title": "Deputy Sports Editor"
        },
        {
          "staff_name": "Heather Chen",
          "title": "Deputy Sports Editor"
        },
        {
          "staff_name": "Sydney Stoller",
          "title": "Deputy Sports Editor"
        },
        {
          "staff_name": "Julian Silva-Forbes",
          "title": "Deputy Sports Editor"
        }
      ],
      "Illustrations": [
        {
          "staff_name": "Ji Yoon Sim",
          "title": "Illustrations Editor"
        },
        {
          "staff_name": "Kelsea Petersen",
          "title": "Deputy Illustrations Editor"
        },
        {
          "staff_name": "Cassie Wang",
          "title": "Deputy Illustrations Editor"
        },
        {
          "staff_name": "Olya Sukonrat",
          "title": "Deputy Illustrations Editor"
        },
        {
          "staff_name": "Julian Michaud",
          "title": "Senior Staff Illustrator"
        },
        {
          "staff_name": "Ren Huang",
          "title": "Senior Staff Illustrator"
        }
      ],
      "Spectrum": [
        {
          "staff_name": "Emma Melnikov",
          "title": "Spectrum Editor"
        },
        {
          "staff_name": "Madeline Suindah",
          "title": "Spectrum Editor"
        },
        {
          "staff_name": "Anuthmi Waas",
          "title": "Deputy Spectrum Editor"
        },
        {
          "staff_name": "Kayla Pham",
          "title": "Spectrum Staff Writer"
        }
      ],
      "Audio": [
        {
          "staff_name": "Claire Schnatterbeck",
          "title": "Director of Podcasting and Audio"
        },
        {
          "staff_name": "Kelly Warner",
          "title": "Audio Staff Reporter"
        },
        {
          "staff_name": "Christina Li",
          "title": "Audio Staff Composer"
        },
        {
          "staff_name": "Matthew Schwitzer",
          "title": "Audio Staff Producer"
        }
      ]
    }
;